...
public class MySQLDataStoreUtilities {
	...
	public static void TruncateProducts(){
			try{
				if(getConnection() == 1){
					...
					String deleteAccessoryQuery = "truncate table accessorys";
		...
		public static void deleteProduct(String p_id){
				try{
					if(getConnection() == 1){
						...
						String deleteAccessoryQuery = "delete from accessorys WHERE p_id='"+p_id+"'";
						PreparedStatement pst1 = conn.prepareStatement(deleteAccessoryQuery);
						pst1.executeUpdate();
		...
		public static int addnewProduct( Product product){
			...
			try{
				...
				String insertAccesoryDetails = "INSERT INTO accessorys(a_id,p_id, a_image, a_name, a_price, a_description) "+ "VALUES (?,?,?,?,?,?)";
		...
		public static int modifyProduct( Product product,String id){
			...
			try{
				if(getConnection() == 1){
					PreparedStatement pst=null;
					String ProductsQuery = "Update products SET name = ?,id = ?,quantity = ?,description = ?,display_under = ?,imageUrl = ?,price = ?,discountAmt = ?,rebateAmt = ? WHERE id='"+id+"'";
					 pst = conn.prepareStatement(ProductsQuery);
					pst.setString(1,product.getName());
					pst.setString(2,product.getId());
					pst.setInt(3,product.getQuantity());
	...
	try{
		if(getConnection() == 1){
			...
			String insertOrderDetailsQuery = "INSERT INTO Orders(O_Id, u_Id, O_Ddate, O_Status, item, price, itemCount, f_name, address, city, state, zipcode, card_number) "+ "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
					...
					for (HashMap.Entry<String, Product> entry : pMap.entrySet()) {
						...
						if(product.getName().equalsIgnoreCase(name)){
							quant = product.getQuantity()-quant;
								String selectOrderDetailsQuery ="UPDATE products SET quantity="+quant+" WHERE name=\""+name+"\"";
								PreparedStatement pst = conn.prepareStatement(selectOrderDetailsQuery);
								pst.executeUpdate();
